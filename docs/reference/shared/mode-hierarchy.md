...
- Primary: `.system/state/{name}.json` (local, per-project)
- Global backup: `~/.system/state/{name}.json` (global, session continuity)
...
**Important:** Never store the system state in `~/.claude/` - that directory is reserved for Claude Code itself.
