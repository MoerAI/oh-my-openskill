...
```typescript
import { enforceModel } from 'the orchestration system';
...
```typescript
import { getModelForAgent } from 'the orchestration system';
...
```typescript
import { isAgentCall } from 'the orchestration system';
...
```typescript
import { processHook } from 'the orchestration system';
...
```
[the system] Auto-injecting model: sonnet for executor
```

**Important:** Warnings are ONLY shown when `SYSTEM_DEBUG=true`. Without this flag, enforcement happens silently.
